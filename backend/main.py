...
app = FastAPI(title="AutoGest API Professional")
...
def criar(dados: dict, db: Session = Depends(get_db), u: str = Depends(get_usuario_logado)):
    novo = Veiculo(marca=dados['marca'], modelo=dados['modelo'], placa=dados['placa'].upper(), valor=dados['valor'], status="EM_ESTOQUE")
    db.add(novo)
    db.commit()
    return novo
...
# --- INTEGRAÇÃO RENAVE (SERPRO) ---
@app.get("/renave/consultar/{placa}")
def consultar_serpro(placa: str, u: str = Depends(get_usuario_logado)):
    """Simulação de consulta ao SERPRO para preenchimento automático"""
    # Na vida real: requests.get(url, cert=(cert, key))
    return {
        "marca": "VOLKSWAGEN",
